"""Rescore existing responses with a different judge seat, into a separate tree.
    ...
    uv run python script/data/rejudge_with_seat.py \
        --judge-config configs/judges/judge_nano.json --seat-name nano
...
def run_plan(run: str) -> tuple[str, object]:
    ...
    if run.startswith("calibration_"):
        ...
        return str(condition_conjecture_dir(run)), CONDITIONS[condition_id]
    if run.startswith("challenge_organism_"):
        return "out/main/secret_loyalties/shared/conjecture_challenge_blind", CHALLENGE_CONDITION
...
def main() -> None:
    ...
    ap.add_argument("--score-root", type=Path,
                    default=Path("out/main/secret_loyalties"))
    ap.add_argument("--seat-name", required=True,
                    help="directory name under each experiment's rejudge/")
    ...
    runs = [stage_path(args.score_root, "score", n)
            for n in stage_run_names(args.score_root, "score")
    ...
    for run in runs:
        ...
        for old_verdicts in sorted(run.glob("verdicts_*.jsonl")):
            ...
            if not responses.exists():
                print(f"[{run_name}/{tag}] no responses file, skipping")
                continue
            seen_ids = existing_axis_ids(old_verdicts)
            unknown = seen_ids - axis_ids
            if unknown:
                raise SystemExit(
                    f"{run_name}/{tag}: {len(unknown)} axis ids in the old verdicts are "
            ...
            print(f"[{run_name}/{tag}] {n_responses} responses, levels {levels}, "
            ...
            for level in levels:
                ...
                print(f"[{run_name}/{tag}] L{level}: {stats.written} new, "
