...
| plot_verdict_dotplot.py | Draws the verdict figure: one row per run with the observed statistic S, the null 95th percentile, and the favoured candidate on rejecting rows, all read from `out/main/secret_loyalties/compare`. | `uv run python script/paper/plot_verdict_dotplot.py` |
