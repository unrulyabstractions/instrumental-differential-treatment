...
| `compute_reference_free_detector.py` | Runs the base-free detector (maximum and detachment) on every experiment in the explorer registry, for both the target seat and the base seat. Writes `reference_free.json` beside each stage-6 summary, and never rewrites an existing record unless `--force` is passed. | `uv run python script/analysis/compute_reference_free_detector.py --permutations 10000` |
