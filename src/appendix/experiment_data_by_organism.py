...
def experiment_data_by_organism_document(out_root) -> str:
    """The whole document body: index, then one appendix per target organism."""
    root = Path(out_root)
    parts = [
        ...
        _index_table(),
        "\\clearpage",
        _calibration_appendix(root),
        "\\clearpage",
        _shared_challenge_appendix(root),
    ]
    for letter in CHALLENGE_TARGETS:
        parts += ["\\clearpage", _challenge_appendix(root, letter)]
    body = "\n".join(parts)
    while "\n\n\n" in body:
        body = body.replace("\n\n\n", "\n\n")
    return body
